...
router.get('/prodtreintisieteredirect/:id', async (req, res) => {
...
router.get('/prodtreintisieteindex/:page', async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : 0);
...
router.get('/addtocardprodtreintisiete/:id', function(req, res, next){
  ...
  Prodtreintisiete.findById(productId, function(err, product){
    if(err){
      return res-redirect('/');
    }
