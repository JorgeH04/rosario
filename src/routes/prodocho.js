...
router.post("/filtroprodocho", function(req, res){
...
router.get('/prodocho/tallecolor/:id',  async (req, res) => {
  var cart = new Cart(req.session.cart ? req.session.cart : {items: {}});

  const prodocho = await Prodocho.findById(req.params.id);
  res.render('prodocho/tallecolor-prodocho', { 
    prodocho,
    products: cart.generateArray(), totalPrice: cart.totalPrice
...
router.post('/prodocho/tallecolor/:id',  async (req, res) => {
  const { id } = req.params;
  await Prodocho.updateOne({_id: id}, req.body);
   const task = await Prodocho.findById(id);
